...
impl RpmNotePlayer {
    ...
    pub fn touch_brandnew_note(
        rctape: &mut RpmCassetteTape,
        rpm_note_ope: &RpmNoteOpe,
        position: &mut Position,
        comm: &Communication,
    ) {
        let board_size = position.get_board_size();
        let pid_opt = if let (_is_legal_touch, Some(piece_identify)) =
            position.touch_beautiful_1note(&rpm_note_ope, comm, board_size)
        {
        ...
        rctape.record_next_note(RpmNote::from_id_ope(pid_opt, *rpm_note_ope));
    }
    ...
    pub fn pop_current_1note_on_record(
        ...
        if let Some(rpm_note) = rpm_record
            .body
            .cassette_tape
            .delete_next(&mut rpm_record.body.ply)
        {
            let board_size = position.get_board_size();
